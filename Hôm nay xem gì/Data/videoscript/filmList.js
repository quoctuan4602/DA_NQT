document.addEventListener("DOMContentLoaded", function () {
  ...
  // Use URLSearchParams to get the `keyword` parameter
  ...
  if (!keyword) {
    ...
    fetchList(`http://localhost:3000/films/search/${keyword}`);
  }
      ...
      .then((data) => {
        ...
        if (data.length === 0) {
          html = `<div class="flex items-center justify-center min-h-screen">
            <h3 class=" text-xl  font-bold rounded">
                Không tìm thấy
            </h3>
          </div>`;
        }
        ...
        if (data.length > 0) {
          ...
          document.getElementById("filmNotfound").innerHTML = html;
        }
      })
      .catch((error) =>
